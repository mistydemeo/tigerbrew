...
class Checks
  ...
  (Keg::TOP_LEVEL_DIRECTORIES + ["lib/pkgconfig"]).each do |d|
    define_method("check_access_#{d.sub("/", "_")}") do
      ...
      by Tigerbrew. If a formula tries to write a file to this directory, the
  ...
  def check_user_path_3
    ...
    if sbin.directory? && sbin.children.length > 0
      unless $seen_prefix_sbin
        <<-EOS.undent
        Tigerbrew's sbin was not found in your PATH but you have installed
  ...
  def doctor
    ...
    methods.each do |method|
      ...
      unless out.nil? || out.empty?
        if first_warning
          puts <<-EOS.undent
            #{Tty.white}Please note that these warnings are just used to help the Tigerbrew maintainers
